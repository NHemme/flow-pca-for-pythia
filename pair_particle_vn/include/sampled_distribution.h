...
public:
    ...
    Sampled_distribution(CDFFunc cdfFunc, T low, T high, T param, unsigned resolution = 200) 
        : mLow(low), mHigh(high), mRes(resolution), mDist(0.0, 1.0)
    {
        if (mLow >= mHigh) throw InvalidBounds();

        mSampledCDF.resize(mRes + 1);
        const T cdfLow = cdfFunc(low, param);
        const T cdfHigh = cdfFunc(high, param);
        T last_p = 0;
        for (unsigned i = 0; i < mSampledCDF.size(); ++i) {
            const T x = i/mRes*(mHigh - mLow) + mLow;
            const T p = (cdfFunc(x, param) - cdfLow)/(cdfHigh - cdfLow); // normalising 
