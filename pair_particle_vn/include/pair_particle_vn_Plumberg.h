...
namespace plumberg
{
	...
	long Nevents;
	const long chosen_multiplicity_per_event = 100;
	...
	vector<vector<double> >
		average_1_by_Npairs_per_event( n_pT, vector<double>( n_pT, 0.0 ) );
	...
	vector<vector<complex<double> > >
		Q_ij_m_Q_i_Q_j( n_pT, vector<complex<double> >( n_pT, 0.0 ) );
	...
	void generate_random_data()
	{
		...
		uniform_real_distribution<double> v2_distribution(0.24, 0.26);
		...
		for (int iEvent = 0; iEvent < chosen_number_of_events; iEvent++)
		{
			...
			{				
				...
				/*const double pT_value = pT_distribution(generator);
				const double v2_at_pT = v2*pT_value / (mean_pT+pT_value);
				const double psi2_at_pT = psi2+0.5*M_PI*v2_at_pT;
				for (int iWeight = 0; iWeight < 36; iWeight++)
					weights[iWeight] = 1.0 + 2.0*v2_at_pT
												*cos(2.0*( 0.5*(intervals[iWeight]+intervals[iWeight+1])
															- psi2_at_pT));
				std::piecewise_constant_distribution<double>
					new_pphi_distribution ( intervals.begin(), intervals.end(), weights.begin() );
...
/*cout << "CHECK: " << event_ID_of_this_particle << "   "
		...
		for ( auto & this_event : all_events )
		{
			...
			if ( Vn_mode == 1 )
				...
				for (int pT_j = 0; pT_j < n_pT; pT_j++)
				{
					...
					const double n_part_i = eta_bin_i.size();
					const double n_part_j = eta_bin_j.size();

					if ( n_part_i > 0 )
						Q_i[pT_i][pT_j] += Q_i_this_event[pT_i][pT_j] / n_part_i;
					if ( n_part_j > 0 )
						Q_j[pT_i][pT_j] += Q_j_this_event[pT_i][pT_j] / n_part_j;
					if ( Npairs_this_event[pT_i][pT_j] > 0 )
						Q_ij[pT_i][pT_j] += Q_ij_this_event[pT_i][pT_j] / Npairs_this_event[pT_i][pT_j];
				}
			else
				for (int pT_i = 0; pT_i < n_pT; pT_i++)
				for (int pT_j = 0; pT_j < n_pT; pT_j++)
				{
					Q_i[pT_i][pT_j] += Q_i_this_event[pT_i][pT_j];
					Q_j[pT_i][pT_j] += Q_j_this_event[pT_i][pT_j];
					Q_ij[pT_i][pT_j] += Q_ij_this_event[pT_i][pT_j];
				}
		}
